...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, 'مقام کاربر ('..name..') به '..value..' تغيير داده شد ', ok_cb,  true)
  end
end
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  text = text..'@Tardteamch'
  send_msg(extra.receiver, text, ok_cb,  true)
  else
	send_msg(extra.receiver, extra.user..' نام کاربري مورد نظر يافت نشد.', ok_cb, false)
  end
end
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  send_msg(extra.receiver, 'ايدي شخص مورد نظر در سيستم ثبت نشده است.\nاز دستور زير استفاده کنيد\n/info @username', ok_cb, false)
  end
end
